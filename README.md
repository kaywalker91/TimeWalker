...
#### 🌐 크로스오버 대화 시스템 개편
- 크로스오버 대화 ID 정규화 (`connect_` → `crossover_` 접두사 통일)
- 새 크로스오버 대화 추가: "인공지능과 사회 변화" (세종 × 다빈치 × 코엑스)
- 기존 "천재들의 만남" 대화를 "타임슬립: 2024년의 서울"로 리뉴얼
- 코엑스 장소에 세종/다빈치 캐릭터 연결

#### 💬 대화 시스템 기능 강화
- 대화 화면 배경 이미지 지원 (장소 배경을 대화 진입 시 전달)
- 캐릭터별 대화 목록 필터링 개선 (dialogueIds 기반으로 크로스오버 대화 포함)
- 대화 선택 시트 잠금 로직 제거 및 UI 간소화

#### ⚙️ 설정 및 개발자 기능
- `GameSettings`에 `developerMode` 필드 추가
- 설정 화면에 Developer Mode 토글 스위치 추가

#### 🧹 코드 정리
- 대화 화면 불필요한 주석 및 중복 로직 제거
- 위치 상세 시트 ListView 구조 개선
- 코드 포매팅 정리
