...
class ChoiceConditionHelper {
  ...
  /// 조건 미충족 메시지 생성
  static String getConditionMessage(ChoiceCondition condition, UserProgress? progress) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: choices.map((choice) {
          final canSelect = ChoiceConditionHelper.canSelectChoice(choice, userProgress);
          
          return Padding(
            padding: const EdgeInsets.only(bottom: 16),
            child: Tooltip(
              message: !canSelect && choice.condition != null
                  ? ChoiceConditionHelper.getConditionMessage(choice.condition!, userProgress)
                  : choice.preview ?? '',
              child: _ChoiceButton(
                choice: choice,
                canSelect: canSelect,
